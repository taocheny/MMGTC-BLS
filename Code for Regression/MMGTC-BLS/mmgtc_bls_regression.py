...
ENHAN_RANGE = range(2, 201, 2)     # N_e : [1,200] 步长 5 → {1,6,...,196}  40 个  (共 8000 组)
...
def run_mmgtc(num_fea, num_win, num_enhan, a1, a2, sigma1, sigma2):
    """用给定架构与核参训练一次 MMGTC-BLS, 返回 (train_err, test_err, n_iter, tr_t, te_t)。"""
    (_, tr_t, te_t, train_err, test_err, n_iter) = mmgtc_bls_train(
    ...
    for num_win in WIN_RANGE:
        for num_enhan in ENHAN_RANGE:
            print(f'[阶段1] 特征节点数 = {num_fea}, 窗口数 = {num_win}, '
                  f'增强节点数 = {num_enhan}')
            try:
                train_err, test_err, _, _, _ = run_mmgtc(
...
pd.DataFrame(arch_records,
             ...
             ).to_excel('arch_results_sunspot06_mmgtc.xlsx', index=False)
...
for a1 in A1_GRID:
    ...
    for sigma1 in SIGMA_GRID:
        for sigma2 in SIGMA_GRID:
            ...
            try:
                train_err, test_err, n_iter, tr_t, te_t = run_mmgtc(
            ...
            if test_err < best:
                ...
                pd.DataFrame(best_dict).to_excel(
                    'best_result_sunspot06_mmgtc.xlsx', index=False)
...
]).to_excel('all_results_sunspot06_mmgtc.xlsx', index=False)
