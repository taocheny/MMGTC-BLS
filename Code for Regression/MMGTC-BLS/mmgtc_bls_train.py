...
# =========================================================================
#  MMGTC-BLS: Broad Learning System via Maximum Mixture Generalized Total
#             Correntropy. 严格按照论文 Eqs. (7)-(13) 实现。
#
...
#    a1 + a2 = 1, 0 < a1,a2 < 1; σ_j 带宽, α_j 形状参数。
#    α=2 → 退化为高斯核 (即 MMTC); α<2 → 重尾, 对 Laplacian/脉冲噪声更鲁棒。
#
#  目标函数 (Eq.9):
#    J2 = argmax_W Σ_i ( a1·τ1·exp(-|ε_i|^α1/σ1^α1) + a2·τ2·exp(-|ε_i|^α2/σ2^α2) )
#                  - (λ/2)·||W||^2
#
#  增广权重向量 (与 MMTC 相同):
#    W̄ = [ sqrt(σ_o^2/σ_i^2) ; -W^T ]^T  ⇒  ||W̄||^2 = σ_o^2/σ_i^2 + ||W||^2
#    σ_i^2, σ_o^2 为输入/输出噪声方差, 本代码均取 0.1, 故 σ_o^2/σ_i^2 = 1。
#
#  闭式解为不动点方程 (Eqs.11-13), 采用不动点迭代求解 (r_i = ||e_i||):
#    Λ2[i,i] = Σ_j (a_j·τ_j·α_j·r_i^{α_j-2}) / (σ_j^{α_j}·||W̄||^{α_j})
#                  · exp( -r_i^{α_j} / (σ_j^{α_j}·||W̄||^{α_j}) )
#    s2      = (1/||W̄||^2) · Σ_i Λ2[i,i]·||e_i||^2
#    ξ2      = λ - s2
#    W(t)    = (U^T Λ2 U + ξ2 I)^{-1} U^T Λ2 Y
#  终止条件: ||W(t) - W(t-1)||^2 < ν (ν=0.05) 或达到最大迭代次数 t_max (=50)。
# =========================================================================
...
def mmgtc_fixed_point(U, Y, a1, a2, alpha1, alpha2, sigma1, sigma2, lam,
    ...
    """
    不动点迭代求解 MMGTC-BLS 的输出权重 W (论文 Eqs. 11-13)。
...
def mmgtc_bls_train(train_x, train_y, test_x, test_y,
    ...
    """
    Maximum Mixture Generalized Total Correntropy Broad Learning System
    (MMGTC-BLS)。隐藏层生成机制与标准 BLS / MMTC-BLS 完全一致, 仅以最大混合
    广义总相关熵准则替代岭回归来优化输出权重。
    ...
    返回:
        (test_pred, training_time, testing_time, train_err, test_err, n_iter)
    """
    ...
    # ==================================================================
    #  不动点迭代求解输出权重 W (论文 Eqs. 11-13)
    # ==================================================================
    start_time = time.time()
    W, n_iter = mmgtc_fixed_point(U, train_y_t, a1, a2, alpha1, alpha2,
    ...
    if verbose:
        ...
        print(f'[MMGTC-BLS] 训练 RMSE = {train_err.item():.4e}')
    ...
    if verbose:
        ...
        print(f'[MMGTC-BLS] 测试 RMSE = {test_err.item():.4e}')
