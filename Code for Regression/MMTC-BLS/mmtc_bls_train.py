...
# =========================================================================
#  MMTC-BLS: Broad Learning System via Maximum Mixture Total Correntropy
#  严格按照论文 Eqs. (1)-(6) 实现
#
#  目标函数 (Eq.1):
#    J1 = argmax_W  Σ_{j=1}^{2} Σ_{i=1}^{N} a_j·exp( -||u_i W - y_i||^2 / (2 σ_j^2 ||W̄||^2) )
#                   - (λ/2)·||W||^2 ,      a1 + a2 = 1,  a_j > 0
#  两个高斯核: σ1 取较小带宽 (梯度陡峭, 精确拟合小残差),
#             σ2 取较大带宽 (快速抑制重尾/大异常值, 提升鲁棒性)。
#
#  增广权重向量 (论文给定):
#    W̄ = [ sqrt(σ_o^2/σ_i^2) ;  -W^T ]^T   ⇒   ||W̄||^2 = σ_o^2/σ_i^2 + ||W||^2
#    其中 σ_i^2, σ_o^2 分别为输入/输出噪声方差, 本代码均取 0.1, 故 σ_o^2/σ_i^2 = 1。
#
#  闭式解为不动点方程 (Eqs.4-6), 采用不动点迭代求解:
#    Λ1[i,i] = Σ_j (a_j/σ_j^2)·exp( -||e_i||^2 / (2 σ_j^2 ||W̄||^2) )
#    s1      = Σ_j Σ_i (a_j/σ_j^2)·(||e_i||^2/||W̄||^2)·exp(·)
#    ξ1      = λ·||W̄||^2 - s1
#    W(t)    = (U^T Λ1 U + ξ1 I)^{-1} U^T Λ1 Y
...
def mmtc_fixed_point(U, Y, a1, a2, sigma1, sigma2, lam,
    ...
    """
    不动点迭代求解 MMTC-BLS 的输出权重 W (论文 Eqs. 3-6)。
...
def mmtc_bls_train(train_x, train_y, test_x, test_y,
    ...
    """
    Maximum Mixture Total Correntropy Broad Learning System (MMTC-BLS)。
    隐藏层生成机制与标准 BLS / BTSK-BLS 完全一致, 仅以最大混合总相关熵准则
    替代岭回归来优化输出权重。
    ...
    # ==================================================================
    #  不动点迭代求解输出权重 W (论文 Eqs. 3-6)
    # ==================================================================
    start_time = time.time()
    W, n_iter = mmtc_fixed_point(U, train_y_t, a1, a2, sigma1, sigma2,
    ...
    if verbose:
        ...
        print(f'[MMTC-BLS] 训练 RMSE = {train_err.item():.4e}')
    ...
    if verbose:
        ...
        print(f'[MMTC-BLS] 测试 RMSE = {test_err.item():.4e}')
