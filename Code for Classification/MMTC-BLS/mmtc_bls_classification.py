...
def run_mmtc(num_fea, num_win, num_enhan, a1, a2, sigma1, sigma2):
    """训练一次 MMTC-BLS 分类器, 返回 (train_acc, test_acc, n_iter, tr_t, te_t)。"""
    (_, tr_t, te_t, train_acc, test_acc, n_iter) = mmtc_bls_train(
    ...
    for num_win in WIN_RANGE:
        for num_enhan in ENHAN_RANGE:
            print(f'[阶段1] 特征节点数 = {num_fea}, 窗口数 = {num_win}, '
                  f'增强节点数 = {num_enhan}')
            try:
                train_acc, test_acc, _, _, _ = run_mmtc(
...
pd.DataFrame(arch_records,
             ...
             ).to_excel(f'arch_results_{DATASET}_mmtc.xlsx', index=False)
...
for a1 in A1_GRID:
    ...
    for sigma1 in SIGMA_GRID:
        for sigma2 in SIGMA_GRID:
            ...
            try:
                train_acc, test_acc, n_iter, tr_t, te_t = run_mmtc(
            ...
            if test_acc > best:
                ...
                pd.DataFrame(best_dict).to_excel(
                    f'best_result_{DATASET}_mmtc.xlsx', index=False)
...
]).to_excel(f'all_results_{DATASET}_mmtc.xlsx', index=False)
